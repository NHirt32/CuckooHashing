...
def rehash(currSize):
    ...
    print(f'Our size pre doubling: {currSize}')
    currSize = currSize * 2
    ...
    print(f'Our next prime size for hashing should be {nextprime(currSize)}')
    table1 = [None] * currSize
    table2 = [None] * currSize
    tableStatePrint(table1, table2)
    maxLoop = 3 * math.log(currSize)
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    hash1 = hashInfo()
    hash2 = hashInfo()

    return (table1, table2, hash1, hash2, maxLoop)
...
def hashInsert(table1, table2, hash1, hash2, key, loopCount, maxLoop, nextPrime):
    loopCount += 1
    ...
    while loopCount <= maxLoop:
        tableStatePrint(table1, table2)
        index = hash1.hFunc(key, len(table1), nextPrime)
    ...
    if loopCount >= maxLoop:    # If our previous while loop
        table1, table2, hash1, hash2, maxLoop = rehash(len(table1))
        return (table1, table2, hash1, hash2, False, maxLoop)
    return (table1, table2, hash1, hash2, True, maxLoop)
...
def cuckooHash():
    ...
    maxLoop = 3 * math.log(currSize)  # Base should be 1 + epsilon, can't access resource tho... function is 3log(r)
    completed = False # We will use this variable to determine if we have rehashed in an insertion attempt
    nextPrime = nextprime(currSize)
    ...
    while completed == False:
        completed = True
        for key in dataSet:
            table1, table2, hash1, hash2, completed, maxLoop = hashInsert(table1, table2, hash1, hash2, key, 0, maxLoop, nextPrime) # Note that unless completed = False hash1 and hash2 will not change
            if completed == False:  # If we have rehashed change our hashing functions and restart the process of inserting keys into our new fresh tables
                nextPrime = nextprime(len(table1))
                break
